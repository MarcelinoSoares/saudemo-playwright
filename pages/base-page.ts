...
export class BasePage {
  ...
  async waitForElement(selector: string, timeout?: number): Promise<Locator> {
    try {
      ...
      console.error(
        `Error waiting for element "${selector}" to be visible: ${error}`,
      );
      throw new Error(
        `Element "${selector}" not visible within ${timeout || 'default'} timeout: ${error}`,
      );
    }
  }
  ...
  async assertElementText(
    ...
    try {
      ...
      throw new Error(
        `Element "${selector}" does not have expected text "${text}" within ${timeout || 'default'} timeout: ${error}`,
      );
    }
  }
  ...
  async fillField(selector: string, value: string): Promise<void> {
    try {
      ...
      console.error(
        `Error filling field "${selector}" with value "${value}": ${error}`,
      );
      throw new Error(
        `Failed to fill field "${selector}" with value "${value}": ${error}`,
      );
    }
  }
  ...
  async assertUrlContains(urlSubstring: string): Promise<void> {
    try {
      ...
      console.error(
        `Error asserting URL contains "${urlSubstring}". Current URL: ${currentUrl}. Error: ${error}`,
      );
      throw new Error(
        `URL does not contain "${urlSubstring}". Current URL: ${currentUrl}. Error: ${error}`,
      );
    }
  }
  ...
  async getAllTextContents(selector: string): Promise<string[]> {
    try {
      ...
      console.error(
        `Error getting text contents from elements matching "${selector}": ${error}`,
      );
      throw new Error(
        `Failed to get text contents from elements matching "${selector}": ${error}`,
      );
    }
  }
}
